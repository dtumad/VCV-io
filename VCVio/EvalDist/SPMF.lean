...
This file defines a type `SPMF` for `PMF`s extended with the ability to failure,
or equivalently having total "mass" less than `1` (with `none` having the missing part).

TODO: This should eventually be moved to mathlib.
-/
...
TODO: should this be reducible. Once we have more lemmas should probably remove. -/
