...
variable {ι : Type u} {spec : OracleSpec ι} {α σ : Type w}
...
NOTE: Use of `WriterT` over `StateT` is probably better than this when possible -/
class IsTracking (so : QueryImpl spec (StateT σ (OracleComp spec))) where
    state_indep : ∀ {α} (q : OracleQuery spec α) s, (so.impl q).run' s = q

namespace IsTracking

@[simp]
lemma run'_simulateT_eq_self [IsTracking so]
    (oa : OracleComp spec α) (s : σ) : (simulateQ so oa).run' s = oa :=
  simulate'_eq_self so (λ _ (query i _) _ ↦ IsTracking.state_indep _ _) s oa

@[simp]
lemma fst_map_run_simulateT_eq_self [IsTracking so]
    (oa : OracleComp spec α) (s : σ) : Prod.fst <$> (simulateQ so oa).run s = oa :=
  IsTracking.run'_simulateT_eq_self so oa s

@[simp]
lemma simulate'_eq_self [IsTracking so]
    (s : σ) (oa : OracleComp spec α) : simulate' so s oa = oa :=
  IsTracking.run'_simulateT_eq_self so oa s

@[simp]
lemma fst_map_simulate_eq_self [IsTracking so]
    (s : σ) (oa : OracleComp spec α) : (Prod.fst <$> simulate so s oa : OracleComp spec α) = oa :=
  IsTracking.run'_simulateT_eq_self so oa s

lemma mem_support_of_mem_support_simulate' [IsTracking so]
