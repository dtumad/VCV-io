...
def join {m : Type _ → Type _} (so : QueryImpl spec₁ m) (so' : QueryImpl spec₂ m) :
...
def append {m₁ m₂ n : Type u → Type v} [MonadLift m₁ n] [MonadLift m₂ n]
...
@[simp] lemma append_apply_inl {m₁ m₂ n : Type u → Type v} [MonadLift m₁ n] [MonadLift m₂ n]
    (so : QueryImpl spec₁ m₁) (so' : QueryImpl spec₂ m₂) (i : ι₁) (t : spec₁.domain i) :
...
lemma simulateQ_liftComp_append_left' (so : QueryImpl spec₁ n) (so' : QueryImpl spec₂ n)
    (oa : OracleComp spec₁ α) : (simulateQ (so ++ₛ so') (liftComp oa (spec₁ ++ₒ spec₂)) : n α) =
      simulateQ so oa := by
  induction oa using OracleComp.inductionOn with
  | pure x => simp
  | query_bind i t oa hoa =>
