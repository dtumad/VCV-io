...
-- instance : Functor (OracleQuery spec) where
--   map := λ {α β} f q ↦ match q with
--     | OracleQuery.mk i t => OracleQuery.mk _ _
...
inductive FreeMonad (f : Type u → Type u) : Type u → Type (u + 1)
  ...
  | roll {α β : Type u} (g : f α) (r : α → FreeMonad f β) : FreeMonad f β

variable {f : Type u → Type u} {α β : Type u}

/-- Bind operator on `OracleComp spec` operation used in the monad definition. -/
protected def FreeMonad.bind : FreeMonad f α → (α → FreeMonad f β) → FreeMonad f β
  | FreeMonad.pure x, g => g x
  | FreeMonad.roll g r, c => FreeMonad.roll g (λ u ↦ FreeMonad.bind (r u) c)

instance : Monad (FreeMonad f) where
  pure := FreeMonad.pure
  bind := FreeMonad.bind

lemma FreeMonad.bind_def (x : FreeMonad f α) (g : α → FreeMonad f β) :
...
inductive OracleComp {ι : Type u} (spec : OracleSpec.{u} ι) : Type u → Type (u + 1)
...
@[simp] protected lemma failure'_eq_failure (spec : OracleSpec ι) (α : Type u) :
...
@[simp] lemma queryBind'_eq_queryBind {α : Type u} (oa : spec.range i → OracleComp spec α) :
...
protected instance instDecidableEq [DecidableSpec spec] [FiniteRange spec]
    [DecidableEq ι] [h : DecidableEq α] : DecidableEq (OracleComp spec α)
